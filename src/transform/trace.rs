use super::common::{
    attributes_from_kv, convert_instrumentation_scope, to_nanos, Attributes,
...
use opentelemetry_types::trace::v1::{span, status, ResourceSpans, ScopeSpans, Span, Status};
use std::collections::HashMap;

fn convert_span_kind(span_kind: SpanKind) -> span::SpanKind {
    match span_kind {
...
fn convert_status_code(status: &trace::Status) -> status::StatusCode {
    match status {
...
fn convert_link(link: Link) -> span::Link {
    span::Link {
        ...
        attributes: attributes_from_kv(link.attributes).0,
        dropped_attributes_count: link.dropped_attributes_count,
        flags: link.span_context.trace_flags().to_u8() as u32,
    }
}
fn convert_span_data(source_span: opentelemetry_sdk::trace::SpanData) -> Span {
    let span_kind: span::SpanKind = convert_span_kind(source_span.span_kind);
    Span {
        ...
        attributes: attributes_from_kv(source_span.attributes).0,
        ...
        events: source_span
            ...
            .map(|event| span::Event {
                ...
                attributes: attributes_from_kv(event.attributes).0,
        ...
        links: source_span
            .links
            .into_iter()
            .map(|l| convert_link(l))
            .collect(),
        status: Some(Status {
            code: convert_status_code(&source_span.status) as i32,
    ...
    ResourceSpans {
        ...
        scope_spans: vec![ScopeSpans {
            schema_url: source_span
                ...
                .unwrap_or_default(),
            scope: Some(
                convert_instrumentation_scope((&source_span.instrumentation_scope, None)).into(),
            ),
            spans: vec![convert_span_data(source_span)],
        }],
    }
}
...
pub fn group_spans_by_resource_and_scope(
    ...
    let scope_spans = scope_map
        ...
        .map(|(instrumentation, span_records)| ScopeSpans {
            scope: Some(convert_instrumentation_scope((instrumentation, None))),
            schema_url: resource.schema_url.clone().unwrap_or_default(),
            spans: span_records
                .into_iter()
                .map(|span_data| convert_span_data(span_data.clone()))
                .collect(),
        })
        .collect();
